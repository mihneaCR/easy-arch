...
======================================================================
${RESET}"
info_print "Welcome to easy-artix, a script made in order to simplify the process of installing Artix Linux."
...
# basestrap (setting up a base sytem onto the new root).
info_print "Installing the base system (it may take a while)."
basestrap -K /mnt base "$kernel" "$microcode" linux-firmware "$kernel"-headers btrfs-progs grub grub-btrfs rsync efibootmgr snapper reflector snap-pac zram-generator sudo &>/dev/null
...
echo "root:$rootpass" | artix-chroot /mnt chpasswd
...
if [[ -n "$username" ]]; then
    ...
    echo "$username:$userpass" | artix-chroot /mnt chpasswd
fi
